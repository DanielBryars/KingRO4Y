...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    /* KingRO4Y modification: some HID peripherals (the Fosi VOL20 among
     * them) don't put the HID service UUID in their advertisement and rely
     * on the HID appearance category (0x03C0..0x03FF) instead. Accept
     * either signal. */
    if (uuid == ESP_GATT_UUID_HID_SVC || (appearance >> 6) == 0x0f) {
